...
const SingleLgaSubjectAnalysisTable = () => {
  ...
  const transformedAnalysis = singleLgaSubjectAnalysis?.JSS3?.map((item) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      fixed: "right",
      render: (_, record) => (
        <Link to={`/dashboard/singleSchool-analysis/${record.id}`}>
...
export default SingleLgaSubjectAnalysisTable;
