...
import { getSingleSchoolSubjectAnalysis } from "@/features/students/studentSlice";
import Loader from "@/lib/Loader";
import Error from "@/lib/Error";
import { useParams } from "react-router-dom";

const SingleSchoolSubjectAnalysis = () => {
  const {
    students,
    isError,
    isSuccess,
  ...
  useEffect(() => {
    dispatch(getSingleSchoolSubjectAnalysis(schoolId));
  }, []);
  ...
  return (
    <Box className="sm:p-5 space-y-4 p-3">
      ...
      {isLoading ? (
        <Loader />
      ) : !isLoading && !singleSchoolSubjectAnalysis ? (
        <Error />
      ) : (
              ...
              <Box className="space-y-4">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {singleSchoolSubjectAnalysis?.examTypes?.[0]?.subjects?.map(
                      (item, index) => {
                        return (
...
export default SingleSchoolSubjectAnalysis;
